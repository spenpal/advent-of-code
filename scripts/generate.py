...
def parse_args() -> Namespace:
    ...
    if args.days == -1:
        parser.error(
            "The -d/--days flag is required outside the Advent of Code "
            "active period (Dec 1 - Dec 25). Please specify a day using "
            "'-d DAY' or a range with '-d START END'.",
        )
    if len(args.days) > 2:
        parser.error(
    ...
    for day in range(start_day, end_day + 1):
        ...
        if not test_file.exists():
            ...
            if examples:
                ...
                print(
                    f"⚠️  LLM extraction failed for day {day}, created empty template: {test_file}",
                )
        else:
