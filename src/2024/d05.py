...
def parse(data: str) -> dict[int, tuple]:
    ...
    ordering = [tuple(map(int, line.split("|"))) for line in ordering.strip().splitlines()]
...
def valid_update(ordering: dict[int, set[int]], update: list[int]) -> bool:
    return all(set(update[i + 1 :]).issubset(ordering[update[i]]) for i in range(len(update) - 1))
...
def part2(ordering: dict[int, set[int]], updates: list[list[int]]) -> int:
    ...
    invalid_updates = [update for update in updates if not valid_update(ordering, update)]
    fixed_updates = [sorted(update, reverse=True, key=lambda num: order(num, update)) for update in invalid_updates]
