...
def packet_parser(packet):
    ...
    if packet_type_id == 4:
        ...
        match packet_type_id:
            case 0: packet_info['value'] = sum(values)
            case 1: packet_info['value'] = math.prod(values)
            case 2: packet_info['value'] = min(values)
            case 3: packet_info['value'] = max(values)
            case 5: packet_info['value'] = 1 if values[0] > values[1] else 0
            case 6: packet_info['value'] = 1 if values[0] < values[1] else 0
            case 7: packet_info['value'] = 1 if values[0] == values[1] else 0
...
def main():
    ...
    with open("puzzle_input.txt") as f:
        ...
        bin_packet = hexToBin(hex_packet)

    print(f"Part 1 Answer: {part1(bin_packet)}")
    print(f"Part 2 Answer: {part2(bin_packet)}")
